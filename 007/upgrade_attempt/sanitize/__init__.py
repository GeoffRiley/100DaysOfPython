...
class BaseHTMLProcessor(html.HTMLParser):
    ...
    def feed(self, data):
        if config.DEBUG:
            sys.stderr.write(f'BaseHTMLProcessor, feed, data={repr(data)}\n')
    ...
    def unknown_starttag(self, tag, attrs):
        ...
        if config.DEBUG:
            sys.stderr.write(f'BaseHTMLProcessor, unknown_starttag, tag={tag}\n')
        ...
        strattrs = ''.join([f' {key}="{attrquote(value)}"' for key, value in newattrs])
    ...
    def handle_data(self, text):
        ...
        if config.DEBUG:
            sys.stderr.write(f'BaseHTMLProcessor, handle_text, text={text}\n')
...
class HTMLSanitizer(BaseHTMLProcessor):
    ...
    def resolveURI(self, uri):
        if ':' in uri:
            ...
            if scheme not in self.acceptable_uri_schemes:
                uri = '#' + rest
        if self.baseuri:
            return urllib.parse.urljoin(self.baseuri, uri)
        else:
            return uri
    ...
    def feed(self, data):
        ...
        while self.tag_stack:
            super().unknown_endtag(self.tag_stack.pop())
...
def _ebcdic_to_ascii(s):
    ...
    if not _ebcdic_to_ascii_map:
        emap = config.EMAP
        _ebcdic_to_ascii_map = str.maketrans(''.join(map(chr, list(range(256)))), ''.join(map(chr, emap)))
